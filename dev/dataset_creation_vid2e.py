...
def build_aprovis_dataset(path, labelz, patch_size = None, sensor_size=None, max_duration=None, min_num_events=1000, train_test_ratio=.75, ordering = 'xytp'):
    ...
    if not os.path.exists(path+f'patch_{patch_size}_duration_{max_duration}'):
        ...
        for _, label in enumerate(labelz):
            print(label)
            list_files = glob.glob(f'./*{label}*/*.npy')
...
class vid2e_Dataset(tonic.dataset.Dataset):
    '''creates a dataset from .npy files in a specific 'path'. Adapted for synthetic events obtained from RGB frames given by  and Jean
    '''
    classes = ["sea", "gro", "mix"]
